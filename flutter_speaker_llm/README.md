...
The repo contains **Dart code only** — the native platform folders
(`android/ios/macos/windows`) are generated by `flutter create` and aren't
committed. A helper script generates them and patches the microphone/network
permissions for you:

```bash
cd flutter_speaker_llm
bash tool/bootstrap.sh          # runs `flutter create` + patches permissions
cd example
...
flutter pub get
# then apply the permissions from "Platform setup" below
flutter run -d macos
```
