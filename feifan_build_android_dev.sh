...
MAX_RETRIES=2
BUILD_COMMAND="flutter build apk --release --target ./lib/main_dev.dart"
attempt=1
while [ $attempt -le $MAX_RETRIES ]; do
    ...
    if [ $? -eq 0 ]; then
        ...
        qrencode -o $(pwd)/build/app/outputs/flutter-apk/qrcode.png -s 12 -m 2 "http://mapptest.feifan.art/apk/app-release.apk"
        exit 0
    else
        echo "Build failed."
        attempt=$((attempt + 1))
